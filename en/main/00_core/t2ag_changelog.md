...
- **Scope**: entry surface translated to English (`README.md`, `AGENTS.md`, the constitution
  `main/t2ag.md`, `first_run.md`, `environment_assumptions.md`, `t2ag_flow.md`, the cloud bridge
  docs, the profile template, and the offline HTML guide). Playbook internals stay zh-CN by
  decision; `README.md` discloses this in a table rather than letting a reader discover it at
  the first Chinese file.
- **Mechanism repairs (why this is not a pure documentation change)**: six gates asserted zh-CN
  string literals against the entry surface, so a correctly-built translated edition failed
  closed for the wrong reason -- the rule was present and the checker was blind. All six were
  widened to accept language variants; every zh-CN literal still passes unchanged.
  - `extract_runtime_version` -- added English prose anchors for the runtime version.
  - `check_cloud_contract` -- Skeleton isolation markers accept language variants.
  - `check_authorization_governance` -- marker moved to `AUTHORIZATION_NON_AMPLIFICATION_MARKERS`.
  - `check_context_packet_contract` -- a workflow marker may be a tuple of accepted variants.
  - `check_course_activity_templates` -- first-run map-first / confirmation preference anchors.
  - playbook protection-level regex -- accepts `**Protection level**:` beside `**保护级别**：`.
- **Portability fixes (found by this work, not caused by it; both reachable in zh-CN too)**:
  - `migrate_021_activity_records.detect_kind` anchored `Skeleton` to end-of-line, so a README
    title suffix such as `(English edition)` silently flipped the detected release kind from
    skeleton to main and probed the wrong tree. Now matched as a word anywhere in the title.
  - `skeleton_package_findings` derived the in-zip prefix from the *archive filename*
    (`stem.split('-0.')[0]`). Any package whose filename did not match its internal top-level
    directory lost every `SKELETON_PRIVACY_EXEMPT` entry and reported a false privacy leak.
    The prefix is now read from the archive's actual single top-level directory. A guard keyed
    on a filename rather than on content is the `carrier_mismatch` family this check exists to
    catch -- it had the defect it was written to prevent.
- **build_guide.py** -- accepts `## Preface` beside `## 序`; flow-card titles and directory-tree
  labels are no longer zh-CN literals.
- **This entry is the state-drift anchor**: the gate repairs changed the atom set and the
  runtime plan, so the anchored assertions below are refreshed.
...
#### 佐证断言
- runtime doctor on a clean-room extraction of the shipped zip = 0 FAIL, 0 WARN ← unzip to an empty dir, run `--profile runtime`
- release doctor identical to the canonical Skeleton baseline (4 FAIL / 4 WARN, all four are t2ag-lite sibling drift in the maintainer workspace) ← same plan sha, both trees
- skeleton privacy = 0 findings across the packaged zip ← manifest `privacy_findings: []`
