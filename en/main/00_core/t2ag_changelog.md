...
## [2026-08-28] LB-PARITY · An English section-budget canonical for this release (no version bump)

- **change**: This release gets its own canonical original for the section-budget mechanism.
  **New file** `50_playbook/line_budget.md` (163 lines), marked `**Protection level**: core-playbook`
  — **an English translation, not a byte copy** of the Chinese editions' file. The mechanism prose is
  then cut to pointers at **four sites**: `70_tools/t2ag_doctor.py` `memory_section_budgets()`
  docstring **paragraph one** (the `[max N]` ownership argument) and **paragraph three** (the v0.1.2
  provenance); the `check_memory_budget` docstring (the WARN-not-FAIL rationale); the
  `check_constitution_budget` docstring (the FAIL-not-WARN rationale); and
  `00_core/t2ag_memory.md` §Section budgets and sinking. Both gates' over-budget remedy messages now
  name the new file. ⛔ **Paragraph two of `memory_section_budgets()` — the definition of a section's
  extent — is kept word for word** (2026-08-27 adjudication two: the parser contract belongs to the
  code, and the canonical file must not restate it). Three things the new file carries that are
  **this release's own facts, not Main's**: §4 names this release's own gate IDs
  (`runtime.memory_budget`, handler `check_memory_budget`, WARN; `runtime.constitution_budget`,
  handler `check_constitution_budget`, FAIL) and states why the file may be in English; §3 does
  **not** claim the sinking criterion and memory's `D-NNN` / `P-NNNN` numbering convention "remain in
  `00_core/t2ag_memory.md`" — this repo's memory carries neither section, so that sentence would be a
  dangling pointer here, and the gap is registered instead; §6 registers this release's own
  migrations as `LB-EN-001`…`LB-EN-006` (`LB-EN-006` = **keep**), ⛔ **not** a copy of Main's
  `LB-001`…`LB-006`, which are Main's batch bookkeeping. Zero check IDs added or removed;
  `70_tools/validation_workflow.json` untouched; no `enforcement:` forward edge written (a carried
  debt, recorded in §4 with its reason). Per `50_playbook/playbook_management.md` §4.2 — core and
  meta playbooks must not be auto-archived, merged, or heavily rewritten, and any modification must
  state its reason in `t2ag_changelog.md` — this entry is **mandatory**, not optional.
- **reason**: Why a translation rather than the byte-identical copy the Chinese editions carry: the
  core-playbook file-set and sha256 comparison in `check_core_playbooks`, and
  `check_playbook_taxonomy_parity`, run only over the editions returned by
  `distribution_release_names()` — `('t2ag', 't2ag-skeleton', 't2ag-lite')` — and **this release is
  not among them**. This file is therefore not on the byte-identical sha surface at all, so shipping
  a Chinese file here would buy nothing and would break this repo's all-English playbook convention.
  Why cut the prose at all: two wordings of one mechanism drift apart independently and **no machine
  watches the seam** — that had already happened once, in the v0.1.2 provenance paragraph, which had
  drifted between editions with no gate reporting it.
  **The trigger and the root cause are not the same thing and must not be recorded as one.** The
  trigger was `check_core_playbooks` reporting a core-playbook **file set divergence** for
  `t2ag-lite`; **the root cause is not the batch that added the file**. `t2ag-lite` has no `.git` and
  is ignored by the workspace `.gitignore`: it is not a peer repository but a whole-tree snapshot
  regenerated by Main's `70_tools/sync_lite.py`, and it has not been regenerated since DEC-0a-2 — it
  still carries the two split gates that Main merged long ago. The previous batch added the 24th
  core-playbook, the stale snapshot did not follow, and that is what **tripped** the set branch. Hard
  evidence: the moment the set branch closes, the sha branch immediately exposes **13** drifted
  core-playbooks (`canon_carrier`, `context_packet`, `doctor_contracts`, `git_workflow`,
  `handoff_management`, `lesson_recover`, `new_course_init`, `progress_tracking`, `session_close`,
  `source_page_assets`, `startup_orchestration`, `t2ag_flow`, `validation_flow`) — **none of which
  this batch touched**; they are the stale snapshot's backlog. The real fix for Lite is a
  `sync_lite.py --write` whole-tree regeneration, which is **blocked on Main's working tree being
  clean** (the script refuses a dirty tree even in check-only mode, rather than project an
  uncommitted intermediate state onto a git-less Lite), and Main is currently dirty mostly from a
  parallel course-authoring session. Hence the 2026-08-28 user adjudication: keep the copied
  `line_budget.md`, ⛔ do not cut Lite's two gate docstrings, and route the real fix to `sync_lite`.
- **validation_entry**: `PYTHONUTF8=1 python main/70_tools/t2ag_doctor.py --profile runtime` run
  **in place in this repo**: **0 FAIL / 9 WARN**, `checks=46`. All nine WARNs are pre-existing
  `RULE-ENF-003` findings on `progress_governance.md` (context anchor text no longer valid) and have
  **zero intersection with this batch's write surface**; no new finding appeared from the new file,
  the trimmed docstrings, or the trimmed memory section. The three anchored values above are
  **bit-identical to the previous entry** and were re-measured this round — this batch changed only
  prose and docstrings and did not touch `validation_workflow.json`. `parse_playbook_protection_levels`
  was called directly on the new file and returns `core-playbook` at line 3, so this repo's own
  core-playbook count is **24**; the new file is therefore recognized by the same parser the parity
  gate uses, while deliberately staying off that gate's comparison surface. For the three release
  editions the static recomputation of `check_core_playbooks` shows core-playbook file sets of
  **24 each and identical**, with `line_budget.md` sha256 equal across them. ⛔ Main's runtime reading
  is **not usable as this batch's reading** — a parallel session is generating course and reading
  activity content there and its counts keep drifting (`D-1459-1`). Write surface, root-cause
  attribution, and the two adjudications are in
  `workspace:docs/handoffs/T2AG_LINE_BUDGET_PARITY_CLOSURE_WORKORDER_2026-08-28.md` §8.
...
## [2026-08-27] RETIRE-微批 · Retire the 0.2.0/0.2.1 migration checks (no version bump)

- **change**: All three repos retire the 0.2.0/0.2.1 migration check surface in one batch.
  `t2ag_doctor.py` drops three dead handlers (`check_migration_evidence`,
  `check_migration_021_evidence`, `check_activity_migration_021_evidence`) and clears their
  three call sites; `validation_workflow.json` drops the check IDs
  `release.migration_020`, `release.migration_021`, `release.activity_migration_021` and
  their release-profile members, taking this repo's `doctor_checks` key set from 66 to 63
  (Main 62 to 59); `contract_test_support.py` drops four handler tests and their four
  `ALL_CONTRACT_TESTS` registrations; `test_legacy_migrations.py` narrows `TESTS` from five
  entries to one. **Two assertions are carried over first, with zero new check IDs**: the thin
  regression guard lands inside the existing `runtime.structure` handler `check_structure`
  as `LEGACY_STUDENT_FLAT_FILES`, a file-level retirement list placed beside `LEGACY_DOMAINS`
  (whose ten original entries are byte-identical), asserting that the four flat student files
  from before the 0.2.1 migration must not reappear under `main/10_student/`; the four
  skeleton assertions that would otherwise lose their carrier (0.2.0 evidence, 0.2.1 profile
  evidence, real ActivityRecord migration evidence, real AR-0001 instance) move verbatim into
  `check_skeleton_privacy`, semantics and messages unchanged. On the Main side only, the two
  `doctor_checks` cardinality literals in `test_distribution_foundation.py` change from 62 to
  59; the same-named file here carries no such literal and is untouched.
  Explicitly retained: `validated_migration_evidence` and its Lite omission binary-SHA proof
  chain via `lite_manifest_sha_for_path`, `test_profile_migration_roundtrip`,
  `MigrationTransactionTests` plus six other retained test classes,
  `test_legacy_migrations.py` itself and the three `migrations.legacy` registrations,
  `PRODUCTION_MIGRATION_APPLY_ENABLED` and the three RT3 authorization-drift assertions, and
  the four locked migrate artifacts.
- **reason**: The three handlers have no live consumer — retirement is judged by the live
  consumer closure, not by file name, location, or version number — so keeping them only made
  the release profile carry three checks that can never produce new information. But they
  carried two classes of assertion that still have live objects: the four skeleton-privacy
  assertions (Main currently holds nine migration evidence files and one AR-0001 instance;
  both skeletons hold zero, so the assertions are actively passing) and the structural guard
  that migration output must not re-enter the active tree. Deleting those with nobody to
  receive them was not acceptable, so they were carried over before the deletion. The guard
  asserts structural existence rather than scanning content tokens, because a token scan would
  immediately red-flag three explicitly retained files. The test cardinality literals move in
  the same batch because changing an assertable count means closing out every one of its
  assertion sites, or the retirement batch turns its own green tests red.
- **validation_entry**: The runtime plan sha256 and `checks` are bit-identical before and
  after (all three deleted checks are release phase — measured, not inferred from the phase);
  the release plan sha256 changes as expected. All three repos report **0 FAIL** for
  `python -B main/70_tools/t2ag_doctor.py --profile runtime`, and across the six runtime and
  release runs the FAIL counts are identical before and after (release side: Main 72/72,
  zh 22/22, EN 53/53). The guard passes on the positive case with 0 FAIL and produces one FAIL
  per planted flat file (four total) on the negative case; the four skeleton assertions each
  go truly red against planted evidence files and a planted AR-0001. Targeted tests are green
  in all three repos; Main `test_distribution_foundation` goes from rc=1 to **rc=0**
  (27 tests). Per-item readings, the sixteen write-surface blob freeze table, patch SHAs, and
  the debt list are in
...
## [2026-08-24] EV-0035 · 0.2.4 L2 rule-anchor migration v2

- Freeze 55 `rule_id` values in eight consumer-driven families and place 68 adjacent HTML-comment
  anchors across 17 canonical owner files. Main, the Chinese Skeleton, and the English Skeleton
  carry the migration in the same batch.
- Doctor now has a bijective `RULE_ANCHORS` / `RULE_MARKERS` registry, strict `has_rule`, and rule
  positioning APIs. `MARKER_VARIANTS` remains an explicit fallback channel. A retained anchor whose
  registered body is gone emits `RULE-ANCHOR-001` WARN.
- Rule consumers and contract assertions now use rule IDs; runtime-output markers retain marker-role
  APIs. Four playbooks use `rule_id` for the `rule_migration` location column. Three-state mutation
  coverage is active, and both named L2 skips now pass.
- **Validation**: 55 rules / 68 placements / bijection true / 0 anchor-without-body; marker robustness
  **12/12**; distribution foundation **9/9** (one conditional skip); context **48/48** (only
  uninitialized-instance skips); runtime Doctor **0 FAIL / 1 WARN**; all **6/6** release atomic test
  files passed, including release contracts **15/15**.
- This batch did not regenerate Lite/Unified, create a Git checkpoint, push, tag, or release. The
  current zh package, Unified projection, and existing tag are stale pending a separately authorized
  rebuild window.
