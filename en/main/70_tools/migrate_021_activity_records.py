...
def detect_kind(repo: Path) -> str:
    ...
    return (
        "skeleton"
        # LV-5 (2026-08-20): anchoring "Skeleton" to end-of-line made release
        # identity depend on the README title having no suffix.  An edition
        # marker such as "(English edition)" silently flipped the detected kind
        # from skeleton to main, and this script then probed the wrong tree.
        # Match the word anywhere in the title instead.
        if repo.name.startswith("t2ag-skeleton")
        or re.search(r"^# T2AG .*\bSkeleton\b", heading, re.MULTILINE)
        else "main"
    )
