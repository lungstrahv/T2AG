...
def derive_current_checkpoint(
    ...
    *,
    ...
    if strict:
        if not header_seen:
            raise ValueError(
                "checkpoint table missing: no header starting with "
                f"`{CHECKPOINT_TABLE_HEADER}` was found"
            )
        if malformed:
            raise ValueError(
                f"checkpoint table has {len(malformed)} rows with an abnormal column count (>=6 required), "
                f"first row: {malformed[0]}"
            )
    return ("none", "none")
...
def replace_frontmatter_fields(content: str, fields: dict[str, str]) -> str:
    ...
    if not lines or lines[0].strip() != "---":
        raise ValueError("progress.md is missing the opening frontmatter delimiter")
    end = next((i for i in range(1, len(lines)) if lines[i].strip() == "---"), None)
    if end is None:
        raise ValueError("progress.md is missing the closing frontmatter delimiter")
    ...
    if remaining:
        raise ValueError(
            f"progress.md frontmatter is missing fields: {sorted(remaining)}"
        )
    return "\n".join(lines)
