...
LITE_README = """# T2AG 0.2.3 online model review snapshot (t2ag-lite)
...
- **Runtime version**: `0.2.3` (textbook teaching delivery boundary defense-in-depth +
  host egress contract; FIN is **not** claimed)
- **Most recent release-qualification baseline**: `0.2.2`, `finalization_delta_passed` on 2026-08-05
- **Changes since**: see the top of `main/00_core/t2ag_changelog.md`.
  **0.2.3 has had no candidate independent re-review and no finalization delta, so it is
...
LITE_AGENTS = """# t2ag-lite 0.2.3 startup notes
...
- Aligned with the source main; the current runtime version is `0.2.3`, and the
  authoritative version number is in `main/t2ag.md`.
- **Baseline and increment**: the most recent release-qualification baseline is `0.2.2` /
  `finalization_delta_passed` (2026-08-05); runtime version `0.2.3` does not claim FIN.
