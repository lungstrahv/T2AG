...
# EV-0022: the production root is no longer bound to a maintainer machine literal; it
# derives from the repo root the code lives in. Any installed instance's repo root is its
# own production root, so the direct_user authorization gate is in force on every instance;
# an explicit --root pointing elsewhere still lands in the test/shadow guard branch
# (the fail-closed semantics are unchanged).
INSTANCE_ROOT = Path(__file__).resolve().parents[2]
# Compatibility alias: older consumers (activity_lifecycle, the doctor environment probe,
# test patches) still read this name.
...
# LV-5 (2026-08-20): the RT3 close-intent vocabulary has ONE definition, in
# activity_ledger.  It used to be duplicated here, and the two copies desynced the
# moment either side changed.  See activity_ledger for the adjudication.
from activity_ledger import (  # noqa: E402
    BOUND_COMPLETED_INTENTS,
    BOUND_COMPLETED_MARKERS,
    BOUND_INCOMPLETE_INTENTS,
    BOUND_INCOMPLETE_MARKERS,
    close_intent_in,
)
RETROSPECTIVE_SECTION_LABELS = {
    "actual_teaching_process": "actual teaching process",
    "content_completion": "course content completion",
    "knowledge_absorption": "knowledge absorption",
    "course_content_feedback": "student course content feedback",
    "teacher_reflection": "teacher teaching reflection",
    "learning_transition": "follow-up learning transition",
}
RETROSPECTIVE_ITEM_LABELS = {
    "taught_content": "what was actually taught",
    "teaching_sequence": "teaching sequence",
    "expanded_or_skipped": "expanded or skipped",
    "plan_difference": "difference from the plan",
    "completed_content": "completed content",
    "unfinished_content": "unfinished content",
    "out_of_scope_content": "out-of-scope content",
    "next_lesson_boundary": "boundary with the next Lesson",
    "initial_understanding": "initial understanding",
    "reasoning_difficulties": "reasoning difficulties",
    "turning_points": "turning points in understanding",
    "self_correction": "self-correction",
    "independent_reconstruction_or_transfer": "independent restatement or transfer",
    "current_mastery": "current mastery",
    "remaining_retests": "still needing retest",
    "valuable_content": "valuable content",
    "difficult_content": "content that was hard to follow",
    "content_sequence": "content order",
    "example_effectiveness": "effectiveness of examples",
    "redundancy_or_omission": "redundancy or omission",
    "requested_course_adjustment": "requested course adjustment",
    "effective_explanations": "effective explanations",
    "overcompressed_expressions": "over-compressed phrasing",
    "over_assistance": "the help boundary",
    "next_teaching_improvement": "improvement for next time",
    "spaced_retests": "spaced retests",
    "next_lesson_entry": "entry point of the next Lesson",
    "learner_thought_followup": "follow-up consumption of the student's ideas",
}
TERMINAL_DECISIONS = {
...
# LV-5: zh-CN spellings of the progress "current progress" summary labels, so a
# terminal close still finds and clears a field written by another edition.
PROGRESS_SUMMARY_LABEL_VARIANTS = {
    "Current teaching page": ("当前讲授页",),
    "Current teaching activity": ("当前教学活动",),
    "Exact stopping point": ("精确停顿点",),
    "Activity conclusion": ("活动结论",),
    "Lesson context": ("Lesson 上下文",),
}


# LV-5: the "current progress" section heading, in every shipped language edition.
CURRENT_PROGRESS_HEADINGS = ("## 二、当前进度", "## 2. Current progress")
...
def render_learner_retrospective(body: dict[str, Any]) -> str:
    ...
    lines = [
        f"# {payload['activity_id']} teaching retrospective",
        "",
        "## Close scope",
        "",
        str((payload.get("close_scope") or {}).get("summary") or "no scope summary provided"),
    ]
    ...
    lines.extend(
        [
            ...
            "",
            f"- completion blockers: {assessment.get('completion_blockers') or 'none'}",
            f"- scope change: {assessment.get('scope_change') or 'none'}",
            f"- reason for the determination: {assessment.get('reason') or 'not provided'}",
            f"- recommended result: `{payload.get('recommendation')}`",
            "",
            "---",
            f"retrospective presentation SHA-256: `{learner_retrospective_sha256(body)}`",
        ]
    )
    return "\n".join(lines)
...
def build_close_body(
    *,
    ...
    completion_reason = (
        "No blocker; scope, evidence and the close tree were each checked; a not_applicable node does not block completion."
        if not blockers and all(mandatory_evidence.values())
        else "A blocker or a missing node in the close tree remains, so completed cannot be recommended."
    )
    body: dict[str, Any] = {
    ...
    *,
    ...
    if tuple_with_comment:
        ...
        intent_matches = (
            ...
            result == "closed_incomplete" and close_intent_in(tail, completed=False)
        )
        if intent_matches:
            return {
    ...
    raise CloseError(
        ...
        "such as `close` / `结课` after the complete retrospective has been presented"
    )
...
def update_progress(
    ...
    *,
    ...
    if kind in {"resume", "confirm_close", "start_activity"}:
        body_text = f"{kind} {next_type}:{next_id}; the structured next_action_* fields govern."
    elif kind == "choose_activity":
        body_text = "choose the next item from several available activities; the structured next_action_* fields govern."
    else:
        body_text = "there is no automatically selected next activity; the structured next_action_* fields govern."
    next_pattern = re.compile(
        r"(?ms)^-\s+\*\*(?:下一步计划|下一步|下次第一件事)\*\*[：:].*?"
        r"(?=^-\s+\*\*|^##\s|\Z)"
    )
    replacement = f"- **Next step plan**: {body_text}\n"
    if next_pattern.search(body):
        body = next_pattern.sub(replacement, body, count=1)
    elif any(h in body for h in CURRENT_PROGRESS_HEADINGS):
        heading = next(h for h in CURRENT_PROGRESS_HEADINGS if h in body)
        body = body.replace(
            heading + "\n",
            heading + "\n\n" + replacement,
            1,
    ...
    if state in {"completed", "closed_incomplete"}:
        terminal_summary = {
            "Current teaching page": (
                f"this {activity_type}:{activity_id} was closed atomically; "
                f"the recovery route is {body_text.split(';', 1)[0]}, and no new teaching has begun."
            ),
            "Current teaching activity": (
                "there is no foreground LearningActivity; "
                f"the ledger terminal state of {activity_type}:{activity_id} is `{state}`."
            ),
            "Exact stopping point": (
                f"the close body, authorization and next-activity snapshot of {activity_type}:{activity_id} are written to the ledger; "
                f"{body_text}"
            ),
            "Activity conclusion": (
                f"{activity_type}:{activity_id} is recorded as `{state}`; "
                "the full conclusion and evidence boundary are governed by the CLR in activity_ledger.md."
            ),
        }
        if activity_type == "lesson":
            terminal_summary["Lesson context"] = (
                "none; currently between activities, with no next Lesson created or activated yet."
            )
        for label, summary in terminal_summary.items():
            # LV-5: a progress.md written by an earlier (or another-language) edition
            # carries the zh-CN spelling of these labels.  Matching only the current
            # edition's spelling would leave the stale field in place and silently
            # skip clearing it -- a terminal close that does not clear the foreground
            # is worse than one that fails loudly.
            for spelling in (label,) + PROGRESS_SUMMARY_LABEL_VARIANTS.get(label, ()):
                pattern = re.compile(
                    rf"(?ms)^-\s+\*\*{re.escape(spelling)}\*\*[：:].*?"
                    r"(?=^-\s+\*\*|^##\s|\Z)"
                )
                if pattern.search(body):
                    body = pattern.sub(f"- **{label}**: {summary}\n", body, count=1)
                    break
    return rebuild_frontmatter(meta, order) + body
