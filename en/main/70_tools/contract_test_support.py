...
def test_profile_container_contract(root: Path) -> None:
    ...
    reset(legacy)
    run_silently(doctor.check_structure)
    assert_message(doctor.fails, "old top-level student profile file still exists")
    assert_message(doctor.fails, "exactly one shared student profile must exist")
    ...
    reset(extra)
    run_silently(doctor.check_structure)
    assert_message(doctor.fails, "top-level directories are not exactly")
...
def test_resume_path(root: Path) -> None:
    ...
    assert_message(doctor.fails, "resume_path is not canonical")
    assert_message(doctor.fails, "resume_path is dangling")
...
def test_progress_identity_is_shared(root: Path) -> None:
    ...
    for field, value in mutations:
        ...
        reset(case_root)
        run_silently(doctor.discover_courses)
        assert_message(doctor.fails, "progress identity contract")
...
def test_teacher_presentation_contract(root: Path) -> None:
    ...
    assert_message(doctor.fails, "teacher template lacks the map-first explanation protocol")
...
def test_checkpoint_projection_is_fail_closed(root: Path) -> None:
    ...
    try:
        ...
        if "abnormal column count" not in str(exc):
...
def test_state_refresh_activity_roundtrip(root: Path) -> None:
    ...
    try:
        ...
        if "explicit activity fields" not in str(exc):
    ...
    try:
        ...
        if "compatibility current_lesson is dangling" not in str(exc):
...
def test_exercise_current_lesson_driver_matrix(root: Path) -> None:
    ...
    for driver in drivers:
        ...
        reset(stale_root)
        run_silently(doctor.discover_courses)
        assert_message(doctor.fails, "compatibility current_lesson is dangling")
...
def test_planned_activity_fields_rejected(root: Path) -> None:
    ...
    reset(illegal)
    run_silently(doctor.discover_courses)
    assert_message(doctor.fails, "planned course canonical-none is invalid")
...
def test_textbook_preparation_activity_matrix(root: Path) -> None:
    ...
    assert_message(doctor.fails, "lacks a preparation Snapshot")
    ...
    try:
        ...
        if "course_id is invalid" not in str(exc):
...
def test_skin_art(root: Path) -> None:
    ...
    assert_message(doctor.fails, "art_file is dangling")
    write(
        root / "main/80_interface/SK001_default/skin.yaml",
        "id: SK001\nname: default\nversion: 1\nwelcome_msg: hello\n"
    ...
    reset(root, "skeleton")
    run_silently(doctor.check_skin_system)
    assert_message(doctor.fails, "does not clearly display t2AG")
...
def test_flow_and_offline_guide(root: Path) -> None:
    ...
    assert_message(doctor.fails, "FLOW set")
    ...
    assert_message(doctor.fails, "is not a ```text character figure")
    assert_message(doctor.fails, "still contains a Mermaid/SVG rendering layer")
    assert_message(doctor.fails, "has too few character figures")
...
def test_offline_guide_version_drift_is_enforced(root: Path) -> None:
    ...
    seed("0.2.3", "0.2.3")
    run_silently(doctor.check_flow_and_guide)
    drifted = [f for f in doctor.fails if "version drift" in f or "lacks the current version marker" in f]
    if drifted:
    ...
    seed("0.2.2", "0.2.3")
    run_silently(doctor.check_flow_and_guide)
    assert_message(doctor.fails, "offline guide version drift")
    assert_message(doctor.fails, "lacks the current version marker")

    reset(root)
    seed("0.2.3", "0.2.0")
    run_silently(doctor.check_flow_and_guide)
    assert_message(doctor.fails, "flow source title version drift")
...
def test_skeleton_package_surface_is_enforced(root: Path) -> None:
    ...
    if not any("maintainer personal information" in item for item in findings):
    ...
    if not any("unreadable" in item for item in doctor.skeleton_package_findings(broken)):
...
def test_exercise_evidence(root: Path) -> None:
    ...
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "lacks the original images")
    assert_message(doctor.fails, "unknown Attempt")
    assert_message(doctor.fails, "lacks a hint-gate snapshot")
...
def test_exercise_activity_links(root: Path) -> None:
    ...
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "Exercise drifts from the activity map ContentGroup")
...
def test_project_completion_evidence(root: Path) -> None:
    ...
    assert_message(doctor.fails, "lacks closure evidence")
    reset(root)
    ...
    assert_message(doctor.fails, "acceptance record referenced by the closure evidence does not exist")
    reset(root)
    write(
        course / "progress.md",
        "## Completion nodes\n\n| node_id | 状态 | 验证模式 | 验收标准 | 关闭证据 |\n"
...
def test_project_completion_step_summary_required(root: Path) -> None:
    ...
    for invalid in ("passed", "passed ·", "passed：   ", "passed · :"):
        ...
        assert_message(doctor.fails, "missing passed + an actual-result summary")
...
def test_textbook_dependency_contract(root: Path) -> None:
    ...
    for malformed in ("MALFORMED-OUTSIDE-GROUP", "", "`NOT-A-NODE`"):
        ...
        assert_message(doctor.fails, "completion-node dependency format is invalid")
    ...
    assert_message(doctor.fails, "dependency crosses out of the content group")
    ...
    assert_message(doctor.fails, "depends on a completion node that does not exist")
...
def test_persistent_exercise_source_contract(root: Path) -> None:
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "problem statement disagrees with the persistent problem source")
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "persistent problem source path is invalid")
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "frontmatter does not match")
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "source document SHA drift")
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "source document path is invalid")
    ...
    assert_message(doctor.fails, "temporary lifecycle domain")
...
def test_activity_map_strict_bidirectionality(root: Path) -> None:
    ...
    reset(root)
    run_silently(doctor.check_exercises, courses)
    assert_message(doctor.fails, "Lesson drifts from the activity map ContentGroup")
    assert_message(doctor.fails, "Lesson uses a retired activity-ownership field")
    assert_message(doctor.fails, "has Lesson/Exercise but lacks an activity map")
...
def test_activity_map_duplicate_and_complete_coverage(root: Path) -> None:
    ...
    assert_message(doctor.fails, "lesson_ids duplicated")
    assert_message(doctor.fails, "Lesson does not appear in the activity map")
...
def test_retired_exercise_ownership_and_sessions(root: Path) -> None:
    ...
    assert_message(doctor.fails, "Exercise uses a retired activity-ownership field")
    assert_message(doctor.fails, "Exercise contains a retired ExerciseSession")
...
def test_lesson_retired_ownership_all_drivers(root: Path) -> None:
    ...
    messages = [
        ...
        if "Lesson uses a retired activity-ownership field" in message
    ]
    if len(messages) != 4:
...
def test_handoff_assertion_without_source_is_reported(root: Path) -> None:
    ...
    write_handoff_fixture(
        repo,
        "## Status\n\n"
        "- the working tree has 89 dirty files\n"
        "- grepping the whole repo for that symbol gives zero hits\n"
    ...
    assert_message(doctor.warns, "handoff assertion has no recomputation source")
    assert_message(doctor.warns, "FIXTURE_HANDOFF.md")
    for token in ("89 dirty files", "zero hits", "sha256"):
...
def test_handoff_assertion_with_source_is_accepted(root: Path) -> None:
    ...
    offenders = [warn for warn in doctor.warns if "handoff assertion has no recomputation source" in warn]
    if offenders:
...
def test_handoff_required_context_layers_are_enforced(root: Path) -> None:
    ...
    run_silently(doctor.check_handoff_contract)
    assert_message(doctor.fails, "lacks the continuity-summary layer")
...
def test_handoff_active_lane_absence_contradiction_is_enforced(root: Path) -> None:
    ...
    run_silently(doctor.check_handoff_contract)
    assert_message(doctor.fails, "registered active while also declaring non-existence")
...
def test_handoff_index_version_drift_is_enforced(root: Path) -> None:
    ...
    run_silently(doctor.check_handoff_contract)
    assert_message(doctor.fails, "handoff index version drift")
...
def test_environment_probes_report_broken_assumptions(root: Path) -> None:
    ...
    assert_message([m for _, m in findings], "never set T2AG_022_CLOSE_TEST=1 just to make apply pass")
    assert_message([m for _, m in findings], "do not auto-install")
    assert_message([m for _, m in findings], "must not clean up on")
...
def test_environment_registry_must_exist_and_list_every_probe(root: Path) -> None:
    ...
    assert_message(doctor.fails, "environment assumptions registry is missing")
    ...
    run_silently(doctor.check_environment_assumptions)
    assert_message(doctor.fails, "environment assumptions registry lacks an entry")
...
def test_changelog_anchor_mismatch_warns_with_both_values(root: Path) -> None:
    ...
    for key, label in (
        ...
        if got != want:
            warns.append(f"state drift with no record: Fixture {label} declared={got} measured={want}")
    if len(warns) != 3:
        raise AssertionError(f"expected three mismatch WARNs, got {warns}")
    assert_message(warns, "declared=" + "a" * 64)
    assert_message(warns, "measured=" + "c" * 64)
    assert_message(warns, "declared=1")
    assert_message(warns, "measured=29")
...
def test_changelog_missing_anchor_block_warns(root: Path) -> None:
    ...
    message = (
        f"the latest changelog entry lacks an anchor block: {entries[0]['heading']}; "
        f"measured plan_sha256={'c' * 64} checks=29 atom_set_sha256={'d' * 64}"
    )
    if "lacks an anchor block" not in message or entries[0]["heading"] not in message:
        raise AssertionError(message)
    if "declared" in message and "measured" not in message:
...
def test_changelog_body_date_disorder_warns(root: Path) -> None:
    ...
    if len(violations) != 1:
        raise AssertionError(f"expected exactly one violation, got {violations}")
    assert_message(violations, "dates are out of order")
...
def test_changelog_stale_evidence_warns_with_title_and_claim(root: Path) -> None:
    ...
    text = _changelog_fixture_body(
        ...
        atom_sha="b" * 64,
        evidence_line=(
            "- the playbook carries a drift trace \u2190 "
            '`grep -c "drift trace" main/50_playbook/changelog_management.md`\n'
        ),
    )
    entries = doctor.parse_changelog_entries(text)
...
def test_memory_budget_over_limit_warns_with_both_numbers(root: Path) -> None:
    ...
    body = _memory_budget_fixture(
        "## Recent key decisions  [max 3]\n" + "".join(f"- entry {i}\n" for i in range(10))
    )
    write(root / "main/00_core/t2ag_memory.md", body)
    run_silently(doctor.check_memory_budget)
    assert_message(doctor.warns, "Recent key decisions")
    assert_message(doctor.warns, "budget 3")
    if not any("measured 11 lines" in w for w in doctor.warns):
...
def test_memory_budget_missing_markers_warns(root: Path) -> None:
    ...
    write(
        root / "main/00_core/t2ag_memory.md",
        _memory_budget_fixture("## Recent key decisions\n- entry\n"),
    )
    run_silently(doctor.check_memory_budget)
    assert_message(doctor.warns, "has no [max N] section budget markers")
...
def test_memory_budget_within_limit_is_silent(root: Path) -> None:
    ...
    write(
        root / "main/00_core/t2ag_memory.md",
        _memory_budget_fixture("## Recent key decisions  [max 50]\n- entry\n"),
    )
    run_silently(doctor.check_memory_budget)
...
def test_memory_budget_counts_only_its_own_section(root: Path) -> None:
    ...
    if title != "Short section" or cap != 5 or actual != 4:
...
def test_rule_enforcement_broken_context_anchor_warns(root: Path) -> None:
    ...
    main = _rgate_fixture(root)
    findings = doctor.rule_enforcement_findings(
        _rule_docs("enforcement: context=t2ag.md#a no-longer-existing anchor#with-hash\n"),
        known_checks=_RGATE_CHECKS,
        main=main,
    )
    if [(code, severity) for code, severity, _ in findings] != [
    ...
    if "a no-longer-existing anchor#with-hash" not in findings[0][2]:
