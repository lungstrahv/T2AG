...
class DoctorBoundaryTests(unittest.TestCase):
    ...
    def test_attempt_date_parser_and_gate_boundary(self) -> None:
        ...
        for value in ("2026-08-01T00:00:00", "banana", "2026-8-1", "2026-02-30"):
            self.assertTrue(any("created is not a valid ISO date" in message for message in errors(value)), value)
        self.assertFalse(any("hint-gate snapshot" in message for message in errors("2026-07-31")))
        self.assertTrue(any("lacks a hint-gate snapshot" in message for message in errors("2026-08-01")))
        self.assertFalse(any("created 非法" in message or "hint gate" in message for message in errors("2026-08-01", "enabled", "none")))
        self.assertTrue(any("fields are not paired" in message for message in errors("2026-07-31", "enabled", "")))
