...
- A `course_type: project` course maps it to a stable Goal/Milestone in its Project Plan; `course.md` owns the plan definition and Completion nodes own node state, with no second plan truth source.
...
- A `course_type: project` course: closed by code that runs, a file produced, or a functional result in its Project Plan, judged by an **external source of truth**. This binds only to `course_type`; Mastery `learning_mode: project` does not enter this closing protocol.
...
| rule_id | rule_id | action | new owner / equivalence gate | consumers | verification |
|---|---|---|---|---|---|
