...
>
> **当前版本**：`0.0.05`
> **产品名**：T2AG（公开名）| **系统代号**：T2AC（内部名，文件命名沿用）
>
> **本文件是什么**：T2AG 系统的**宪法与结构清单**，不再是种子文件。
> 它规定不变的核心原则，并登记系统全部部件（每个一行）。
> **再生系统请用 `T2AC_skeleton`**（空白骨架，天生是新一代种子）。
> 日常运行只读 `00_core/t2ac_memory.md`；本文件仅在两个时刻被完整阅读——
...
- **再生入口**：新实例 = 解压 `T2AC_skeleton` + 按第三章清单核对。
- **运行时事实**：热文件是 memory；本文件低频阅读，故必须保持最新——所有日常文件的合法性都溯源到它。
- **当前版本**：`0.0.05`（版本号须与 AGENTS.md / README.md 一致，doctor 检查）。
...
### 2.1 权威链
各课程 `course_status.md` 是进度**唯一真相源**；`t2ac_memory.md` 指针与 `course_info.md` 进度列均为**缓存**。冲突时以真相源为准：先跑 `70_tools/t2ac_doctor.py`，向学生口头核对，再修复、再开课。

### 2.2 教师红线
- **情绪红线**：开课前必读学生情绪记录（`00_core/t2ac_emo.md` → `10_case/students/Sxxx/`），据此调节语气、速度、压力；不得无视学生状态硬推。
...
| 部件 | 路径 | 职能 | 定义 | 检查 |
|---|---|---|---|---|
| 宪法 | `main/t2ac.md` | 本文件 | 自身 | 行数/版本一致 |
| 记忆索引 | `00_core/t2ac_memory.md` | 跨会话热文件 | 自身 | 分节预算 |
| 变更史 | `00_core/t2ac_changelog.md` | 唯一史官 | 自身 | — |
| 问题日志 | `00_core/t2ac_problemlog.md` | 踩坑记录 | 自身 | — |
| 情绪索引 | `00_core/t2ac_emo.md` | 学生状态索引 | 自身 | — |
| 复利回路模式 | `00_core/pattern_retire_loop.md` | 根因-退役模板 | 自身 | 实例声明 |
| 流程图 | `00_core/t2ac_flow.md` | 开课/结课流程 | 自身 | — |
| 课程组规则 | `00_core/course_group_rules.md` | 课程识别+组管理 | 自身 | doctor 四检 |
| 考试规则 | `00_core/exam_rules_final.md` | 考试子系统总纲 | 自身 | doctor 四检 |`n| 项目日程表 | `00_core/project_schedule.md` | 向前看路线图 | 自身 | — |

### 10_case —— 师生与课程配置
| 部件 | 路径 | 职能 | 定义 | 检查 |
|---|---|---|---|---|
| 案例总览 | `10_case/t2ac_case.md` | 培养方案树 | 自身 | 存在 |
...
### 4.2 agent 接管（开课）
启动读取顺序：本文件 → `00_core/t2ac_memory.md` → `10_case/t2ac_case.md` →
`teacher_overlay.md` → `student_info.md` → `course_info.md` →
（按指针）课程组/实践 →（按需展开）changelog/problemlog。
详细开课流程见 `00_core/t2ac_flow.md`。读完展示欢迎信息再教学。

### 4.3 缺文件再生
任一启动文件缺失时，按 skeleton 对应文件再生空模板并提示回填；
`t2ac.md` 缺失是致命的，提示用户从 skeleton 恢复。授权边界：再生只补结构，不得伪造学生真实数据。
