...
>
> **但仪式不是强制**：学生有权随时走 micro close 快速离场。完整仪式是默认路径，不是惩罚门槛。
...
> **设计哲学**：学生进入系统是想学习的；整理归档是 agent 的家务，不是学生的债。

**触发条件**（满足任一）：
- 学生表达离开意图但对话即将中断（如"先走了""下次再说"后无后续回应）
- 对话因外部原因意外中断（网络断开、token 耗尽）
- 学生明确表示不想走完整仪式

**只做两件事**：
1. **记停点**：在 `course_status.md`「当前进度」写入精确停顿点（页码+概念+一句话），并在文件头标记 `close_type: micro`
2. **写 memory 一行**：在 `t2ag_memory.md`「上次课摘要」追加一行，格式：`[日期] [课程代码] micro close → 停在「XXX」，待整理`

**不做**：question_bank 核对、mistake_bank 收割、学生状态写回、course_info 刷新、课后提炼、Git。

**遗留整理**：下次开课时，`lesson_recover.md` 步骤 5.5 检测到 `close_type: micro` 后，agent 静默补齐全部遗漏步骤（1.5→2→3→4→5），不向学生报告"你上次欠了什么"，整理完毕后清除 `close_type: micro` 标记。
...
将步骤 1–7.5 的写入结果与 doctor 运行结果合并为**一张表**一次性展示：
...
- doctor：0 FAIL / X WARN（列出）
- git：未启用 / 待提交「文件清单」/ commit「……」/ push 已同步
[案例指定句尾]
```
