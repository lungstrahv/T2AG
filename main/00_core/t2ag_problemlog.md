...
## 维护规则（problem log governance）

### 1. 什么时候必须查

- 执行 OCR、教材下载、文件迁移、课程初始化、课程恢复、doctor 修复、规则升级前：先查 `main/50_playbook/`，再用关键词检索本文件。
- 遇到与历史问题相似的错误提示、乱码、缺文件、路径不一致、缓存与真相源冲突时：先读相关条目，再动手修。
- 规划新课程、估算课时或改动教学结构时：若担心重复踩坑，查本文件中同类规划/结构问题。

### 2. 什么应该写入

- 工具/环境问题：OCR、PDF、下载、依赖、PATH、编码、文件占用。
- 结构/规则问题：文件层级、命名、初始化、自举、权威链、doctor 规则、playbook 过时。
- 可迁移的用户个性化需求：会影响未来 agent 行动方式的偏好、红线、流程要求。
- 不确定但高风险的判断：本次没有完全解决，但下次必须知道风险在哪里。

### 3. 什么不写入

- 学生的知识性错误：写入对应课程 `mistake_bank.md`。
- 课程进度和停顿点：写入对应课程 `course_status.md`。
- 纯情绪状态：按 `t2ag_emo.md` 与学生档案规则写入。
- 无复用价值的闲聊、重复确认、一次性偏好。

### 4. 条目字段

每条记录至少包含以下字段；旧条目可保持历史格式，不强行重写。
...
**解决**：最终如何解决，或当前状态（已解决 / 悬而未决 / 需要用户决策）。

**后续**：因此修改了哪些文件、规则或流程；是否需要更新 `t2ag_memory.md` 或 playbook。
