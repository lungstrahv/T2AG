...
| 40_field_practices/S002/FP-S002-0001_TradingDiscipline/trade_journal | 条目 | 交易 | **事前** | 决策规则层 | 月复盘→改系统 | 同标签两月未犯→退役 | 同标签复发→重新入账 |
| 00_core/t2ag_problemlog | 条目 | 系统运维 | 事后 | 流程/工具层 | 修 playbook→改系统 | 同类不再发 | 同类故障再现→重开条目 |

> 三个衰减实例的共同点：**恢复力都在文件内**（开课抽查 / 月复盘 / 修复义务）——这是衰减型的入册门槛。
...
> 预告：FieldPractice 的证据索引形态上是 Praxis 课程验收回路的流量台账，
> 第二阶段 P002 迁移落地时按本节入册。
...
3. 兼容期：旧三行声明（无子型、五参数）→ WARN 提示升级，不 FAIL；升级完成一批后收紧为 FAIL。
...
- FP-S002-0001 证据台账待 IV1001 消费关系成文后入册。
# 复利回路模式（Retire-Loop Pattern）

> T2AG 第一个正式设计模式。凡“记录→归因→状态维护→退出密集处理”结构的文件皆为本模式实例。
> **命名法则：模式定义一次于此，实例保留领域名**（trade_journal 不改叫 mistake_bank_trading）。
...
| ⑤退出条件 | 何时退出密集处理 | 进入维护/陈年/归档，或同标签连续 N 周期未犯 |

## 现有实例登记表

| 实例 | 域 | ②时机 | ③归因层 | ④消费方 | ⑤退出密集处理 |
|---|---|---|---|---|---|
| 各课 mistake_bank | 知识点 | 事后 | 概念层 | 开课抽查→改理解 | 三次独立正确→maintenance；六次未过→aged |
| 40_field_practices/S002/FP-S002-0001_TradingDiscipline/trade_journal | 交易 | **事前** | 决策规则层 | 月复盘→改系统 | 同标签两月未犯 |
| 00_core/t2ag_problemlog | 系统运维 | 事后 | 流程/工具层 | 修 playbook→改系统 | 同类不再发 |
| Cocoon taste.md 反馈环 | 信息品味 | 事后 | 品味条款层 | 反馈脚本→改 taste | （程序自治，仅登记） |
...
> 【参数】域=XXX｜时机=XXX｜归因层=XXX｜消费方=XXX｜退出=XXX
