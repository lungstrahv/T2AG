...
- **入口声明**：调用启动协议前分别声明 `entry.teach|entry.maintain|entry.audit|entry.release` 与
  `session_lane`；缺 token 不得默认 `entry.teach`。教学、维护、只读审查、发行任务依次使用
  同名入口；入口合同 canonical：`main/50_playbook/startup_orchestration.md`（§十一–§十四）。
- **启动**：四入口只共用宪法、欢迎信息与授权 kernel；只有 `entry.teach` 启动课程恢复与
  textbook Scope。编队、命令、handoff 字段与两阶段汇合 canonical：同文件 §零–§五。
