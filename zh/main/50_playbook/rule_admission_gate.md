...
复发到阈值的问题不得再以散文收尾——**阈值、计数基准与字段语义的正文在
`00_core/t2ag_problemlog.md` 头部「出局规则」，此处不复制**，只记指针。复制一份就会漂移，
漂移之后没人知道哪份算数：本节 2026-08-22 至 08-26 就是该漂移的实例（出局规则改判后
本节未跟，旧谓词 `occurrence_count >= 2` 悬挂四日，2026-08-26 去谓词修复）。

`context=` 类虽然只报 WARN，**升级压力照旧**：同一条 context 锚反复失效，说明它靠的
那份上下文本身不稳定，复发到阈值后应当换成 `check=` / `tool=`，或如实降为
