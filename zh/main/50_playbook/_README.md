...
- `main/00_core/gate_index.md`：Main-only 门索引数据（managed_by: process_governance），已迁出本目录。T2AG 全部门的一张表——有向图 + 每道门管什么 +
  ...
  自身 `enforcement: prose_accepted`——**它是索引不是约束，无机器兜底**；不随 Skeleton 分发。
...
- `progress_governance.md`：进度呈现治理（meta-playbook，EV-0034，PG-D0=A）。Learner Surface /
  Operator Surface 分界与 Meaningful Pause 的裁决正本——学生看见什么、以什么顺序、何时
  必须停顿、内部 ID/SHA/诊断何时可见。**只管呈现**：进度事实仍归 `progress_tracking.md`，
  门语义归 `gate_index.md`，过程对象归 `process_governance.md`。八旅程面证据在工作区 PG0
  报告；PG-D1 已裁保持三门，PG-D2 已裁默认渐进展示并保留完整遍历。renderer 面由
  `contracts.learner_journey` 落点，Operator envelope 由 `contracts.operator_result` 落点；
  PG9 已完成 Main／中文／英文三 Edition 投影。
