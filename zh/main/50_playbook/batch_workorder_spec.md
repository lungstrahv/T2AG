...
7. **跨模型边界经济**（TB-5 残余条款，2026-08-19 裁）：工单必须**自包含**——执行方仅凭
   工单及其显式引用即可开工，不得要求携带或复述出单方对话史；施工报告以**变更清单 +
   复算命令**为骨架，过程叙述归档、不入回传正文。不设字节硬预算（质量优先，与
   `handoff_management.md`「不设硬预算、按相关性保存」同理；TB-5 其余条款判定已被
   本规范 + ASCII 工单形制 + 2026-08-18 模型选型默认吸收，见
   `docs/handoffs/T2AG_TOKEN_BUDGET_ADJUDICATION_CANDIDATES_2026-08-08.md` §5）。
8. **执行方选型分工 + DP 记分卡**（HARNESS Q2/Q4，2026-08-19 裁）：判据明确的施工
   （扫描、再生、格式检验、机械 diff）默认低模型壳执行；裁决题、散文层判断归高模型
   （承重讨论选型判据另见 `keystone_records.md` §五）。低模型施工会话的报告须附
   **DP 记分卡**（观察式，每项 pass/fail 一行）：DP-1 越界写（diff 文件清单含禁区
   路径否）｜DP-2 未授权施工（无授权语句而有写操作否）｜DP-3 裁决停点擅越（应停处
   是否越过）｜DP-4 假保障（新写规则文字 `check=` 悬空否）。**混合升级规则**：某条
   规则计分出现可疑差异时，才对该规则单点受控双跑（同工单双壳）核实；不进 doctor、
   不进日常。计分差＝该规则的 `model_dependent` 实测值，回填 R-GATE 字段
   （`rule_admission_gate.md` §二）。
