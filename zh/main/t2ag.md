...
| `60_journal/` | 只追加历史、施工报告与归档原文；决策生命周期见 Evolution Register |
| `docs/adr/` | 可移植架构决定（ADR）；生命周期 owner 为 Evolution Register |
| `70_tools/` | doctor、状态刷新、迁移、索引与派生工具 |
| `80_interface/` | 皮肤、欢迎文本与界面资产 |

`bin/` 是命令入口，不属于编号域。`cloud/` 是仓库级同步边界。`docs/` 是仓库级
非编号域，持有 handoff、ADR 与施工报告；ADR metadata 见 `docs/adr/README.md`，
handoff 生命周期归 `50_playbook/handoff_management.md`。`.venv`、
...
#### 块间过渡协议（block transition protocol）

确认门闭合后、下一块内容出现前，必须依次经过以下三门，不得跳过任一步：

1. **闭合声明** — 明确宣布当前块的确认门已闭合，标注闭合依据（学生正确回答了什么）。
2. **感受/疑问门** — 询问学生对这一步的感受或是否有疑问。必须等待学生回应；学生只说"继续"
   或"没问题"即视为该门通过。
3. **下一块授权门** — 指出下一个块的**唯一**名称（如"例 1.2.1"），询问是否进入。必须等待
   学生明确说"继续""好""进入"或同类授权。

禁止事项：

- **禁止跳过感受门**：学生答对确认题后直接进入下一块内容，等同于跳过感受门，违规。
- **禁止合并授权**："继续"不授权整节课、整页或跨块——只授权下一块。学生说"继续"后
  教师若未按上述三门逐一执行而连续展示多块，违规。

允许事项：

- **短期知识结构预告**：过渡语中可以给出"接下来依次是例 1.2.1 → 例 1.2.2 → 三要素"这类
  导航性预告，帮助学生建立局部结构感。但每轮只能实际展开一个块的内容，其余块保持
  名称提及即可，不得提前展示其完整正文、例题或推导。

此协议是 §1.6 三门协议在块间过渡的具体执行条款，与 §1.6 同等优先。
